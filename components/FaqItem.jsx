  ...
  return (
    <div className='w-full border-b'>
        ...
        <div className='flex items-center justify-between py-6'>
            <h4 className="h4 max-w-[300] sm:max-w-md md:max-w-max">{title}</h4>
            <button 
                className='w-[44px] h-[44px] bg-accent flex items-center justify-center'  onClick={() => setIsOpen(!isOpen)}
             >
                {isOpen ? (
        ...
        >
        <p className='pb-8 flex itens-center max-w-[860px]'>{description}</p>
        </div>    
    </div>
  );
};
